...
def train(mode, loader, model, se, optimizer=None, scheduler=None):
    ...
    all_batch_y = []
    memory_loss = torch.nn.TripletMarginLoss(margin=1.0)
    for index, (x_batch, y_batch, time_stamp_batch, edge_index) in enumerate(loader):
        ...
        y_, x_org, pos, neg  = model(x_batch,se,time_stamp_batch,edge_index)
        ...
        loss_memory_1 = memory_loss(x_org, pos, neg)
        loss_memory_2 = mse_loss(x_org, pos)

        loss = loss_y + loss_memory_1 * 0.01 + loss_memory_2 * 0.01
        # loss = loss_y
