...
class myDataset(Dataset):
    def __init__(self, features, targets, edge_index, time_slots:list, augmented=False) -> None:
        super().__init__()
    ...
    def __getitem__(self, index:int) -> torch.Tensor:
        if self.augmented:
            if index < len(self.features):
                ...
                return self.features[index], self.targets[index], self.time_slots[index:index+24], self.edge_index
            else:
                # return augmented data
                index = index - len(self.features)
                return self.features_aug[index], self.targets_aug[index], self.time_slots_aug[index:index+24], self.edge_index
        else:   
            return self.features[index], self.targets[index], self.time_slots[index:index+24], self.edge_index
...
class process_dataloader(object):
    ...
    def _generate_task(self, num_timesteps_in: int = 12, num_timesteps_out: int = 12):
        ...
        self.features_train, self.targets_train = self.seq2sample(self.X[0:point1])
        self.time_slots_train = self.time_slots[:point1+24]
        # data augmentation
        self.features_train_aug, self.targets_train_aug = self.seq2sample(self.X[0:point1:self.args.aggregation_size])
        ...
        self.features_vali, self.targets_vali = self.seq2sample(self.X[point1:point2])
        self.time_slots_vali = self.time_slots[point1:point2+24]
        self.features_test, self.targets_test = self.seq2sample(self.X[point2:])
        ...
        # for i, j in indices:
        #     self.features.append((self.X[i : i + num_timesteps_in,:, :]))
        #     self.targets.append((self.X[i + num_timesteps_in : j,:, :]))

        # data augmentation
        # every aggregation_size, we extract one point
        # self.X_aug = self.X[0::self.args.aggregation_size]
        # self.time_slots_aug = self.time_slots[0::self.args.aggregation_size]
        # self.features_aug, self.targets_aug = [], []
        # # generate seqs len == 24
        # indices = [
        #     (i, i + (num_timesteps_in + num_timesteps_out))
        #     for i in range(self.X_aug.shape[0] - (num_timesteps_in + num_timesteps_out) + 1)
        # ]

        # for i, j in indices:
        #     self.features_aug.append((self.X_aug[i : i + num_timesteps_in,:, :]))
        #     self.targets_aug.append((self.X_aug[i + num_timesteps_in : j,:, :]))
        # return
    ...
    def get_dataset(
        ...
        if self.args.aggregation_size is not None:
            ...
            train_loader = DataLoader(myDataset((self.features_train, self.features_train_aug),
            ...
            train_loader = DataLoader(myDataset(self.features_train, self.targets_train,
                                                self.edges_index,
        ...
        vali_loader = DataLoader(myDataset(self.features_vali, self.targets_vali,
                                           self.edges_index,
        ...
        test_loader = DataLoader(myDataset(self.features_test, self.targets_test,
                                           self.edges_index,
...
if __name__ == "__main__":
    class dummyArgs():
        def __init__(self) -> None:
            ...
            self.layer_num_g = 3
            self.epoch_num = 100
            self.save_path = f"./runs/model_test/{self.dataset_name}/test_{datetime.datetime.today().strftime(r'%Y-%m-%d-%H-%M')}"
            self.resume = True
            self.checkpoint_path = f"./runs/model_test/{self.dataset_name}/check_point"
    args = dummyArgs()
    data_loaders = load_multi_domain(args)
    load_SE(args, "METR_LA")
    ((train_loader, vali_loader, test_loader), SE) = data_loaders["METR_LA"]
